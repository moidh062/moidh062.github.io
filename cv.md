---
layout: page
title: CV
permalink: /cv/
---

You can find my curriculum vitae/resume below.
<ul>
<!-- 	<li><a href="long_cv.pdf">CV</a> (4 pages)</li> -->
	<li><a href="cv/CV of Moidul Hasan.pdf">Long resume</a> (2 pages)</li>
<!-- 	<li><a href="short_cv.pdf">Short resume</a> (1 page)</li> -->
</ul>

<h2>CV Highlights</h2>

<h3>🎓 Education</h3>
<hr>
<p>
  <strong>BSc. in Biomedical Engineering</strong><br>
  Bangladesh University of Engineering and Technology (BUET), Dhaka<br>
  <em>Feb 2020 – Mar 2025</em> | <strong>GPA:</strong> 3.49/4.00
</p>
<p>
  <strong>HSC</strong><br>
  Notre Dame College Dhaka, Dhaka<br>
  <em>2017 – 2019</em> | <strong>GPA:</strong> 5.00/5.00
</p>

<h3>🔬 Research Experience</h3>
<hr>
<p>
  <strong>Deep Learning Models for Tuberculosis Detection from Chest X-rays</strong><br>
  <em>Jan 2024 – Present</em><br>
  – Improved model interpretability using knowledge distillation<br>
  – Localized TB regions using explainable AI<br>
  – Enhanced generalization across datasets
</p>

<p>
  <strong>View-Retina: Low-cost Smartphone Ophthalmoscope</strong><br>
  <em>Jul 2022 – Nov 2023</em><br>
  – Designed a smartphone ophthalmoscope for early glaucoma detection<br>
  – Automated cup-to-disc ratio via image processing
</p>

<p>
  <strong>Diabetic Retinopathy Detection</strong><br>
  <em>Jul 2022 – Feb 2023</em><br>
  – Built a deep learning model for referable DR classification<br>
  – Developed a diagnostic web app
</p>

<h3>💼 Skills</h3>
<hr>
<p>
  <strong>Languages:</strong> Python, C, MATLAB<br>
  <strong>Frameworks:</strong> PyTorch, TensorFlow, Scikit-learn<br>
  <strong>Tools:</strong> SolidWorks, AutoCAD, COMSOL, Ansys Fluent, LaTeX, MS Office
</p>

<h3>🎖️ Achievements</h3>
<hr>
<ul>
  <li>1st Runner-up, University Innovation Hub Program (UIHP) (2025)</li>
  <li>Winner, BME Fest 2.0 Healthcare Case Competition (2024)</li>
  <li>Best Enzyme Award, Bangladesh Biology Olympiad Dhaka South (2022)</li>
</ul>

<h3>🤝 Voluntary Involvement</h3>
<hr>
<ul>
  <li><strong>Assistant Organizing Secretary</strong>, BDBO Dhaka South <em>(Dec 2021 – Mar 2025)</em></li>
  <li><strong>Vice President</strong>, BUET BMES Student Chapter <em>(Dec 2021 – Mar 2025)</em></li>
  <li><strong>Co-Founder & VP (Comms)</strong>, BUET Animata <em>(Nov 2023 – Mar 2025)</em></li>
  <li><strong>Event Management Chair</strong>, BUET ACS Student Chapter <em>(Jan 2024 – Mar 2025)</em></li>
</ul>
